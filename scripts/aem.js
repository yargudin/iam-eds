...
function sampleRUM(checkpoint, data) {
    ...
    try {
        ...
        if (!window.hlx.rum || !window.hlx.rum.collector) {
            ...
            const id =
                (window.hlx.rum && window.hlx.rum.id) ||
    ...
    if (scriptEl) {
        try {
            ...
            if (scriptURL.host === window.location.host) {
                ...
                [window.hlx.codeBasePath] = scriptURL.href.split(
                    '/scripts/scripts.js',
                );
            }
        } catch (error) {
    ...
    block.querySelectorAll(':scope > div > div').forEach((blockColumn) => {
        if (blockColumn.hasChildNodes()) {
            const hasWrapper =
                ...
                validWrappers.some(
                    (tagName) =>
                        blockColumn.firstElementChild.tagName === tagName,
                );
            if (!hasWrapper) {
                wrap(blockColumn);
            } else if (
                blockColumn.firstElementChild.tagName === 'PICTURE' &&
                (blockColumn.children.length > 1 ||
                    !!blockColumn.textContent.trim())
            ) {
                wrap(blockColumn);
            }
        }
...
function decorateSections(main) {
    main.querySelectorAll(':scope > div:not([data-section-status])').forEach(
        (section) => {
            const wrappers = [];
